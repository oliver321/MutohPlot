...
.queue-card,.history-card{grid-column:1/-1}.jobs{display:grid;gap:.5rem}.queue{display:grid;gap:.5rem;overflow-x:auto}.job,.queue-item{display:grid;grid-template-columns:2fr 1fr 1.5fr auto;gap:.7rem;padding:.7rem;border-radius:8px;background:#e7eee9;align-items:center}.queue-item{min-width:820px}.job strong,.job span,.queue-item strong,.queue-item span{overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.job .complete{color:#176b4c}.job .error,.job .cancelled{color:#a52a2a}.queue-actions{display:flex;gap:.35rem;flex-wrap:nowrap;white-space:nowrap}.queue-actions button{width:auto;margin:0;padding:.45rem .65rem;white-space:nowrap}.queue-actions button:first-child,.queue-actions button:nth-child(2){min-width:2.4rem;padding:.45rem}.queue-actions .remove{background:#7d4038}
