...
$('calcalculate').onclick=()=>{const fields=['caltop','calbottom','calleft','calright'];if(fields.some(id=>$(id).value.trim()===''))return $('calresult').textContent='Bitte alle vier Abstände in mm eingeben';const values=fields.map(id=>Number($(id).value.replace(',','.')));if(values.some(value=>!Number.isFinite(value)||value<0))return $('calresult').textContent='Bitte nur positive Millimeterwerte eingeben';const [top,bottom,left,right]=values,papers={a3:[297,420],a2:[420,594],a1:[594,841],a0:[841,1189]},[width,height]=papers[$('paper').value],drawableWidth=width-left-right,drawableHeight=height-top-bottom,first=-(top-bottom)/2,second=-(left-right)/2;if(drawableWidth<=0||drawableHeight<=0)return $('calresult').textContent='Die Messwerte ergeben keine gültige Zeichenfläche';$('calresult').textContent=`Gemessene Zeichenfläche ${drawableWidth.toFixed(1).replace('.',',')} × ${drawableHeight.toFixed(1).replace('.',',')} mm · Mittelpunktkorrektur Achse 1 ${first.toFixed(2).replace('.',',')} mm · Achse 2 ${second.toFixed(2).replace('.',',')} mm`}
function calibrationPayload(){return{name:$('calprofilename').value,paper:$('paper').value,window:$('calwindow').value,top_mm:$('caltop').value,bottom_mm:$('calbottom').value,left_mm:$('calleft').value,right_mm:$('calright').value}}
async function loadCalibrationProfiles(selected){const data=await api('/api/calibration/profiles'),select=$('calprofiles');select.replaceChildren();const names=Object.keys(data.profiles);if(!names.length){const option=document.createElement('option');option.value='';option.textContent='Keine gespeichert';select.append(option);$('caldelete').disabled=true;return}for(const name of names){const profile=data.profiles[name],option=document.createElement('option');option.value=name;option.textContent=`${name} · ${profile.paper.toUpperCase()} ${profile.window}`;select.append(option)}select.value=selected&&data.profiles[selected]?selected:names[0];$('caldelete').disabled=false;select.onchange=()=>{const p=data.profiles[select.value];$('calprofilename').value=p.name;$('paper').value=p.paper;$('calwindow').value=p.window;$('caltop').value=p.top_mm;$('calbottom').value=p.bottom_mm;$('calleft').value=p.left_mm;$('calright').value=p.right_mm;$('calresult').textContent=`Gespeichert · Zeichenfläche ${String(p.drawable_width_mm).replace('.',',')} × ${String(p.drawable_height_mm).replace('.',',')} mm · noch nicht aktiv`};select.onchange()}
