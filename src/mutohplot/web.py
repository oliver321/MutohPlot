...
<label>Papierformat</label><select id="paper"><option value="a3">A3 · Standard</option></select><small>Standardformate und gespeicherte kalibrierte Formate stehen gemeinsam zur Auswahl.</small>
...
let token=null,localMessage='',penMap={},mappingType='',mappingProfilePens={},profileData=null,editingOriginal=null,plotStatus='idle',plotStarted=false,previewBusy=false,previewQueued=false,queueBusy=false,measuredHardClip=null,hardwareData=null,calibrationProfilesData=null; const $=id=>document.getElementById(id);
...
async function loadCalibrationProfiles(selected){const data=await api('/api/calibration/profiles'),select=$('calprofiles'),paperSelect=$('paper'),previousPaper=paperSelect.value;calibrationProfilesData=data;select.replaceChildren();paperSelect.replaceChildren();const names=Object.keys(data.profiles),standardGroup=document.createElement('optgroup');standardGroup.label='Standardformate';for(const paper of ['a3','a2','a1','a0']){const option=document.createElement('option');option.value=paper;option.textContent=`${paper.toUpperCase()} · Standard`;standardGroup.append(option)}paperSelect.append(standardGroup);if(names.length){const calibratedGroup=document.createElement('optgroup');calibratedGroup.label='Kalibrierte Formate';for(const name of names){const profile=data.profiles[name],option=document.createElement('option');option.value=`cal:${name}`;option.textContent=`${name} · ${profile.paper_width_mm} × ${profile.paper_height_mm} mm${name===data.active?' · AKTIV':''}`;calibratedGroup.append(option)}paperSelect.append(calibratedGroup)}paperSelect.value=data.active?`cal:${data.active}`:(['a3','a2','a1','a0'].includes(previousPaper)?previousPaper:'a3');$('caldeactivate').disabled=!data.active;if(!names.length){const option=document.createElement('option');option.value='';option.textContent='Keine gespeichert';select.append(option);$('caldelete').disabled=true;$('calactivate').disabled=true;return}for(const name of names){const profile=data.profiles[name],option=document.createElement('option');option.value=name;option.textContent=`${name} · ${profile.paper_width_mm} × ${profile.paper_height_mm} mm · ${profile.window}${name===data.active?' · AKTIV':''}`;select.append(option)}select.value=selected&&data.profiles[selected]?selected:(data.active||names[0]);$('caldelete').disabled=false;$('calactivate').disabled=false;select.onchange=()=>{const p=data.profiles[select.value],active=p.name===data.active;$('calprofilename').value=p.name;$('calpaper').value=p.paper;$('calwindow').value=p.window;$('calpaperwidth').value=p.paper_width_mm;$('calpaperheight').value=p.paper_height_mm;$('caltop').value=p.top_mm;$('calbottom').value=p.bottom_mm;$('calleft').value=p.left_mm;$('calright').value=p.right_mm;$('calresult').textContent=`Gespeichert · Blatt ${String(p.paper_width_mm).replace('.',',')} × ${String(p.paper_height_mm).replace('.',',')} mm · Zeichenfläche ${String(p.drawable_width_mm).replace('.',',')} × ${String(p.drawable_height_mm).replace('.',',')} mm · ${active?'AKTIV':'nicht aktiv'}`};select.onchange()}
async function setPlotPaper(){const value=$('paper').value,name=value.startsWith('cal:')?value.slice(4):null;await api('/api/calibration/profiles/activate',{name});await loadCalibrationProfiles(name);localMessage=name?`Kalibriertes Format ${name} ausgewählt`:`Standardformat ${value.toUpperCase()} ausgewählt`;$('status').textContent=localMessage;await status();requestPreview()}
...
$('paper').onchange=()=>setPlotPaper().catch(e=>{localMessage=e.message;$('status').textContent=e.message});
