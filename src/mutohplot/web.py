...
.queue-card,.history-card{grid-column:1/-1}.jobs,.queue{display:grid;gap:.5rem}.job,.queue-item{display:grid;grid-template-columns:2fr 1fr 1.2fr 3fr;gap:.7rem;padding:.7rem;border-radius:8px;background:#e7eee9;align-items:center}.job strong,.job span,.queue-item strong{overflow:hidden;text-overflow:ellipsis}.job .complete{color:#176b4c}.job .error,.job .cancelled{color:#a52a2a}.queue-actions{display:flex;gap:.35rem;flex-wrap:wrap}.queue-actions button{width:auto;margin:0;padding:.45rem .65rem}.queue-actions .remove{background:#7d4038}
...
async function loadQueue(){try{const data=await api('/api/queue'),box=$('queue');box.replaceChildren();if(!data.queue.length){box.textContent='Keine vorbereiteten Aufträge';return}for(const [index,item] of data.queue.entries()){const row=document.createElement('div');row.className='queue-item';const name=document.createElement('strong');name.textContent=`${item.position}. ${item.name}`;const profile=document.createElement('span');profile.textContent=item.profile_name;const size=document.createElement('span');size.textContent=item.plot_width_mm==null?'–':`${String(item.plot_width_mm).replace('.',',')} × ${String(item.plot_height_mm).replace('.',',')} mm`;const actions=document.createElement('div');actions.className='queue-actions';for(const [action,label] of [['up','Nach oben'],['down','Nach unten'],['remove','Entfernen'],['start','Plotten']]){const button=document.createElement('button');button.textContent=label;if(action==='remove')button.className='remove';button.disabled=item.status!=='prepared'||(action==='up'&&index===0)||(action==='down'&&index===data.queue.length-1);button.onclick=()=>queueAction(item,action).catch(e=>{$('status').textContent=e.message});actions.append(button)}row.append(name,profile,size,actions);box.append(row)}}catch(e){$('queue').textContent=e.message}}
